...
#if((ACTIVE_PROJECT == PROJECT_DEPHY) && (ACTIVE_DEPHY_PROJECT == PRJ_DEPHY_EXO))
#ifdef BOARD_TYPE_FLEXSEA_MANAGE
					...
					uint16_t *len, uint8_t offset)
{
	...
	shBuf[index++] = offset;
		...
		#if((ACTIVE_PROJECT == PROJECT_DPEB31) || ((ACTIVE_PROJECT == PROJECT_DEPHY) && (ACTIVE_DEPHY_PROJECT == PRJ_DEPHY_EXO)) && RUNTIME_FSM1 == ENABLED)
		ri = &dpRigid;
